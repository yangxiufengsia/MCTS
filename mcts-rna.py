...
def MCTS(root, itermax, k, verbose = False):
    ...
    for i in range(itermax):
        ...
        for i in range(50):
            ...
            mutated_seq1=update(paired_pos,dif_ini,posl,posbase)
...
def update(paired_pos,dif_ini,posl,predicted_seq):
    ...
    for i in range(len(final_even)):
        ...
        if predicted_seq[final_even[i]]=="A":
            #print predicted_seq
            #if predicted_seq[final_odd[i]]==""
            predicted_seq[final_odd[i]]="A"
        if predicted_seq[final_even[i]]=="U":
            if predicted_seq[final_odd[i]]=="G":
                predicted_seq[final_odd[i]]="C"
            if predicted_seq[final_odd[i]]=="A":
                predicted_seq[final_odd[i]]="U"
            #predicted_seq[final_odd[i]]=random.choice(U_change)
        if predicted_seq[final_even[i]]=="C":
            predicted_seq[final_odd[i]]="C"
        if predicted_seq[final_even[i]]=="G":
            if predicted_seq[final_odd[i]]=="U":
                predicted_seq[final_odd[i]]="A"
            if predicted_seq[final_odd[i]]=="C":
                predicted_seq[final_odd[i]]="G"
    ...
    for i in range(len(even_ini)):
        predicted_seq[odd_ini[i]]=random.choice(["A","U","C","G"])
        if predicted_seq[odd_ini[i]]=="G":
            #print predicted_seq[even_ini[i]], predicted_seq[odd_ini[i]]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-pk',default=0, help="this is for handling pseduoknot structures, you can use different pseduoknot prediction software, value=1 means choose pKiss")
